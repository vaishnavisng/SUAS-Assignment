                ...
                <tbody>
                  {filteredTrades.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>{t.signalDate}</td>
                      <td>
                        {t.entryDate} <span style={{ color: 'var(--text-secondary)' }}>(₹{t.entryPrice})</span>
                      </td>
                      <td>
                        {t.exitDate} <span style={{ color: 'var(--text-secondary)' }}>(₹{t.exitPrice})</span>
                      </td>
                      <td style={{ color: t.isWin ? 'var(--success)' : 'var(--danger)', fontWeight: 600 }}>
                        {t.netReturnPct >= 0 ? '+' : ''}{t.netReturnPct}%
                      </td>
                      <td style={{ color: t.isWin ? 'var(--success)' : 'var(--danger)' }}>
                        {t.pnlAmount >= 0 ? '+' : ''}₹{t.pnlAmount.toLocaleString()}
                      </td>
                      <td>{t.daysHeld}d</td>
                      <td style={{ color: 'var(--text-secondary)' }}>{t.exitReason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
